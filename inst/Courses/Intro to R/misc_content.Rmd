...
Let's load some real data and test out some of our new tools. R comes preloaded with various datasets. To see a list of them, type `library(help = "datasets")`.

---

We're going to use the `trees` dataset. Type `?trees` to view the documentation.
...
Type `str(trees)` to see the "structure" of this dataset.
...
We'll discuss more ways of working with datasets in future lessons, but for now we're just going to use the data from the `Volume` variable of this dataset to practice 
...
Let's create a simple logical vector and explore some of its properties. Assign the vector `c(TRUE, FALSE, TRUE, NA)` to a new variable called `log_vect`.
